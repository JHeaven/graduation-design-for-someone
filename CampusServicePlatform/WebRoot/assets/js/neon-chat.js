...
;(function($, window, undefined)
{
	...
	//在此加入websocket握手代码
	var ws = null;  
    var url = 'ws://' + window.location.host + '/CampusServicePlatform/websocket';
    //组件url
    //alert("url:"+url);  
    if (!url) {  
        alert('url error!');  
    ...
    ws.onmessage = function (event) {
    	...
    	//alert(requestdata);
    	//var requestdatasite = requestdata.indexOf("$");
    	//alert("requestdata:"+requestdata+",requestdatasite"+requestdatasite);
    	var toUserId = event.data.substring(0,event.data.indexOf("$"));
    	var toUserMessage = event.data.substring(event.data.indexOf("$")+1);
    	//alert(toUserId+"====="+toUserMessage);
    	//neonChat.pushMessage(id, message, from, time, fromOpponent, unread)
    	if(event.data.indexOf(":")>0){
    		//alert(requestdata);
    		toastr.success(requestdata,null,notificationOpts);
    		//neonChat.refreshUserIds();
    		//neonChat.pushMessage(toUserId, toUserMessage.replace( /<.*?>/g, '' ), $chat.data('current-user'), new Date());
    	}
